...
import { ShowModal } from "app/components/ShowsModal";
					...
					shows.map((movie) => (
						<div key={movie?.id} className="relative group">
							<Image
								...
								alt={movie?.title || ""}
								className="rounded-lg cursor-pointer transform transition-transform duration-300 hover:scale-105"
								width={500}
								height={750}
								priority
								onClick={() => setSelectedMovie(movie)}
							/>
							<div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-75 text-white text-sm p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-b-lg">
								{movie?.overview}
							</div>
						</div>
					))}
